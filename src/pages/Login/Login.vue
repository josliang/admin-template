                    ...
                    <div class="flex justify-center items-center space-x-[10px]">
                        <svg viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" width="45"
                            ...
                            <path
                                ...
                                :fill="appStore.appTheme"></path>
                        </svg>
                        <n-gradient-text :gradient="gradient" class="text-[24px]">Admin</n-gradient-text>
                    </div>
                    <div class="text-[#5a6f7e]">Admin 中后台解决方案</div>
                </div>
                <n-form ref="formRef" :model="formModel" :rules="formRules" label-placement="left" size="large"
                        class="w-[100%]" :show-require-mark="false" :show-label="false">
                    ...
                    <n-form-item>
                        <n-button type="primary" size="large" round @click="handleSubmitForm" :loading="submitLoading"
                                  block>登录
                        </n-button>
                    </n-form-item>
                </n-form>
...
import {mockTempToken} from "@/mock/index";
...
const handleSubmitForm = (e) => {
    ...
    formRef.value.validate(async (errors) => {
        if (!errors) {
            const loadingCallback = message.loading("登录中...");
            submitLoading.value = true;
            let res = await mockTempToken(formModel);
            if (res.code === 200) {
                //登陆之前清除store已有数据，防止冲突
                appStore.clearAllState(() => {
                    loadingCallback.destroy();
